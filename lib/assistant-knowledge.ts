...
export function getKnowledgeBundle(): string {
  ...
  md.push("");
  md.push("### Signed explainer videos — use them generously");
  md.push("PI has a library of real interpreters explaining the most common Deaf-attendee questions, in BSL and ISL. When the user asks one of these questions, call `getSignedExplainer` and embed the returned URL inline as a Markdown link — the chat widget will render it as an inline video player with controls. The model needs to do TWO things in one response: write the short text answer AND include the video URL.");
  md.push("");
  md.push("Topics in the catalogue (pass these as the `key` parameter):");
  md.push("- `how-to-book` — how to book an access ticket (8 chapters)");
  md.push("- `know-rights` — your rights as a Deaf attendee, UK + Ireland (8 chapters in BSL, 5 in ISL)");
  md.push("- `orientation` — tour of the PI Events App (7 chapters)");
  md.push("- `categories` — browsing event categories");
  md.push("- `search` — searching for events");
  md.push("- `request` — requesting an interpreter");
  md.push("- `booking` — general booking guidance");
  md.push("- `faqs` — common FAQs (need to ask first? how early? cost? bring someone? where will the interpreter be?)");
  md.push("- `tips` — five practical tips for accessing events");
  md.push("- `at-event` — what to do at the event (show staff, ordering, emergencies, speech-to-text)");
  md.push("- `notifications` — setting up app notifications");
  md.push("- `volunteer` — volunteering with PI");
  md.push("");
  md.push("**Language**: pass `BSL` by default. Pass `ISL` if the user has signalled they're in the Republic of Ireland or asked for ISL. (Northern Ireland — pass BSL unless they ask otherwise.)");
  md.push("**chapterIndex**: optional 0-based index. If the question matches a specific chapter (e.g. \"how do I get a companion ticket?\" → `know-rights` chapter 5 \"Companion tickets\"), pass it. Otherwise omit and the video plays from the start.");
