...
export function getKnowledgeBundle(): string {
  ...
  md.push("> Most UK venues offer a free companion or PA ticket for Deaf and disabled attendees, but the rules vary venue-by-venue. The free ticket is usually for someone supporting you on the day; if you bring extra friends or family, they pay normal price. Check the specific venue's access page or email their access team — the event detail page on [/events](/events) has the venue's contact info.");
  md.push("");
  ...
  md.push("");
  md.push("**Festivals (Glastonbury, Reading, etc.)** — \"Do you do festivals?\"");
  md.push("> Yes — festivals are one of PI's core sectors. Whether a specific festival has BSL provision depends on the festival operator. Check [/events](/events) for confirmed festival dates with interpreters, or [/events/request](/events/request) to request access at one that hasn't booked it.");
  md.push("");
  md.push("**Sport / Arsenal / football** — \"Are there interpreters at sport?\"");
  md.push("> Sport is one of PI's sectors. Specific sports fixtures with interpreters are listed on [/events](/events). For a fixture not listed, use [/events/request](/events/request) — the venue is legally required to consider reasonable adjustments under the Equality Act 2010.");
  md.push("");
  md.push("**Panto / kids' shows / theatre** — \"Are there interpreters at panto?\" / \"Theatre access for my Deaf child?\"");
  md.push("> Theatre and family shows are common BSL/ISL events. Some productions have an integrated signed performance; others bring an interpreter for specific dates. Check [/events](/events) for dates near you, or use [/events/request](/events/request) to request access at a specific show.");
  md.push("");
  md.push("**Glastonbury / Wembley specifics** — Wembley Stadium runs a partnership with PI to provide BSL at every concert. For other venues, check the event detail page on [/events](/events) for the venue's access contact.");
  md.push("");
  md.push("**Where to sit / sightlines** — \"Where do I sit for the BSL interpreter?\"");
  md.push("> Seating with sightline to the BSL interpreter is set by the venue's access team — it's usually a designated section close to the interpreter platform. Email the venue's access team (look up the contact via the event detail page on [/events](/events) or ask me to look up the venue) and request access seating.");
  md.push("");
  md.push("**Complaints about access** — \"The venue refused BSL — what do I do?\"");
